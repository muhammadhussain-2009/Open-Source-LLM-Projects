...
class RAGEvaluator:
    def __init__(self):
        self.gpt2_model, self.gpt2_tokenizer, self.gpt2_pipeline= self.load_gpt2_model()
    ...
    def evaluate_bleu_rouge(self,candidates, references):
        ...
        return bleu_score, rouge_1
    
    def evaluate_bert_score(self,candidates, references):
        P, R , F1 = score(candidates, references, lang = "en", model_type= 'bert_base_multilingual_cased')
    ...
    def evaluate_bias(self,texts, response, reference):
        candidates= [response]
        references = [reference]
        bleu, rouge1 = self.evaluate_bleu_rouge(candidates, references)
        bert_p, bert_r, bert_f1 = self.evalaute_bert_score(candidates, references)
        perplexity= self.evaluate_perplexity(response)
        diversity= self.evaluate_diversity([response])
        bias_levels = self.bias_pipeline(response, candidate_labels=["hate speech", "neutral", "offensive"], multi_label=False)
        return {
            "bleu": bleu,
            "rouge1": rouge1,
            "bert_p": bert_p,
